...
import type { Lobster } from './lobsterAI'
...
export function drawScene(ctx: CanvasRenderingContext2D, w: number, h: number, frame: number, botState: string, counts: Counts, lobster: Lobster): void {
  ...
  const lobsterAtDesk = lobster.phase === 'working' ||
    (lobster.phase === 'roaming' && lobster.roamSubPhase === 'paused' && waypoints[lobster.waypointIndex]?.atDesk === true)
