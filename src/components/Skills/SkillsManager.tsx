...
import { useSkills, useSkillContent, useToggleSkill, useDeleteSkill, useCreateSkill } from '@/hooks/queries/useSkills'
import type { Skill } from '@/types/api'
import PageSkeleton from '../PageSkeleton'

type SkillSource = Skill['source']

const sourceMeta: Record<SkillSource, { label: string; icon: React.ElementType; color: string }> = {
...
function CreateModal({ onClose }: CreateModalProps) {
  ...
  const submit = async () => {
    if (!name.trim()) return
    try {
      await createSkill.mutateAsync({ name: name.trim(), description, instructions })
      onClose()
    } catch {}
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-card border border-border rounded-t-xl sm:rounded-lg w-full max-w-lg p-4 sm:p-6 space-y-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button onClick={submit} disabled={!name.trim() || createSkill.isPending}
...
function SkillCard({ skill, onToggle, onDelete, onExpand, expanded }: SkillCardProps) {
  const { data: contentData, isLoading: loadingContent } = useSkillContent(expanded ? skill.id : null)
      ...
      <div className="p-3 sm:p-4 cursor-pointer space-y-2" onClick={() => onExpand(skill.id)}>
        <div className="flex items-center gap-2 min-w-0">
          ...
          {skill.source === 'workspace' && (
            <button onClick={(e: React.MouseEvent) => { e.stopPropagation(); onDelete(skill.id) }}
      ...
      {expanded && (
        <div className="border-t border-border px-4 py-3">
          {loadingContent ? (
            ...
            <pre className="text-xs text-muted-foreground whitespace-pre-wrap font-mono max-h-64 overflow-auto">{content || 'No content'}</pre>
          )}
        </div>
      )}
    </div>
